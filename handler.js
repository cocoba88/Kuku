...
const sendMessage = (conn, m, text) => {
  conn.sendMessage(m.chat, text, 'conversation', { quoted: m })
}
...
module.exports = {
    async handler(chatUpdate) {
        ...
        //    const groupMetadata = (m.isGroup ? (conn.chats[m.chat].metadata || await conn.groupMetadata(m.chat)): {}) || {}
            ...
            const isBotAdmin = bot && bot.admin || true // Are you Admin?
            for (let name in global.plugins) {
                let plugin = global.plugins[name]
                if (!plugin) continue
                if (plugin.disabled) continue
...
global.dfail = (type, m, conn) => {
    let tdan = 'https://telegra.ph/file/b8058998404b007758b43.jpg'
    let msg = {
