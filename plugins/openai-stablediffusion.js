...
let handler = async (m, { text, conn, usedPrefix, command }) => {
  ...
  const response = await fetch(`https://api.botcahx.live/api/search/stablediffusion?text=${text}&apikey=eCRUrjCZ`);
...
handler.command = handler.help = ['diffusionx', 'stablediffusionx', 'diffxx'];
handler.tags = ['tools'];
handler.limit = true;
